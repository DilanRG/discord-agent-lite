...
schema ceiling is 7. A database reporting a newer schema is rejected rather
than downgraded.
...
- `messages`: bounded user/assistant turns, scope, Discord message ID, author
  identity, and proactive marker. The unique `(scope, discord_message_id)`
...
- `profile_facts`, `agent_journal`, `relationships`, and reflection events:
  separate global social continuity keyed by immutable user ID.
...
Schema-7 databases can also contain compatibility tables from older releases:
...
`/memory storage enabled:false` disables future conversation-message and
explicit-memory writes for that user throughout the current guild, or in that
user's DM scope. It does not erase existing rows and does not opt out of the
separate agent-owned social continuity.

`/memory forget confirm:true` deletes conversation messages and explicit
memories belonging to that user throughout the current guild, or in that
user's DM scope. It advances the conversation revision and invalidates
...
provider receives bounded pair records, not attachments, quoted reply context,
or unrelated third-party chatter. The save boundary validates provenance,
...
See [MIGRATION.md](../MIGRATION.md) for the v1.1-to-schema-7 procedure and
